...
import { ArrowRight, Shield, Clock, Truck, Phone, ChevronRight, CheckCircle, HeartPulse } from "lucide-react";
...
const saimzContent: HeroContent = {
  ...
  ctaLink: "/contact",
  features: ["Licensed Pharmacists", "Free Delivery", "Same-Day Service", "Insurance Accepted"],
  stats: [
    { icon: <Clock className="h-4 w-4 sm:h-5 sm:w-5" />, label: "Hours", value: "Mon–Sat 9-6" },
    { icon: <Truck className="h-4 w-4 sm:h-5 sm:w-5" />, label: "Delivery", value: "Free Local" },
    { icon: <Phone className="h-4 w-4 sm:h-5 sm:w-5" />, label: "Call", value: "+1-973-782-4444" },
  ]
};
...
export function HeroSlider({ brand }: HeroSliderProps) {
  ...
  // حل مشكلة Hydration - تشغيل فقط على الكلاينت
  useEffect(() => {
    setIsMounted(true);
  }, []);
  ...
  // ============================================================
  // ===== SAIMZ HERO (معدل لحل مشكلة Hydration) =====
  // ============================================================
  if (isSaimz) {
    return (
      <div 
      ...
      >
          ...
          <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 py-12 sm:py-0">
            <div className="max-w-3xl">
              ...
              >
                ...
                <span className="font-mono text-[10px] sm:text-xs font-medium uppercase tracking-wider text-white/80">
              ...
              >
                {content.stats.map((stat, idx) => (
                  <div key={idx} className="flex items-center gap-2 sm:gap-2.5 group cursor-default">
              ...
              >
                {content.features.map((feature, idx) => (
                  <div key={idx} className="flex items-center gap-1 sm:gap-1.5">
                    ...
                    <span className="text-[10px] sm:text-xs text-white/50 font-medium">{feature}</span>
                  </div>
                ))}
                <div className="flex items-center gap-1 sm:gap-1.5">
                  <Shield className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-[#4A9FFF]" />
                  <span className="text-[10px] sm:text-xs text-white/50 font-medium">NPI #1962995290</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="absolute bottom-4 sm:bottom-8 right-4 sm:right-8 z-20 hidden lg:flex flex-col items-center gap-2">
          <span className="text-[10px] font-mono uppercase tracking-wider text-white/20">Scroll</span>
  ...
  return (
    <div ref={containerRef} className="relative h-screen w-full overflow-hidden bg-ink">
      ...
      <div className="absolute bottom-20 left-20 z-5 h-80 w-80 rounded-full bg-sage/10 blur-3xl animate-pulse hidden sm:block" style={{ animationDelay: "2s" }} />
            ...
            >
              ...
              <div className="flex flex-wrap items-center justify-center sm:justify-start gap-1.5 sm:gap-3">
                <span className="inline-flex items-center gap-1 sm:gap-1.5 rounded-full backdrop-blur-sm px-2.5 sm:px-4 py-1 sm:py-1.5 font-mono text-[8px] sm:text-xs font-medium uppercase tracking-wider text-amber-50 border border-amber/30">
                  ...
                  <span className="hidden xs:inline">Independent Community Pharmacy</span>
              ...
              <div className="flex items-center gap-3 sm:gap-6 pt-1">
                ...
                <p className="text-[8px] sm:text-xs text-white/50 truncate">
                  Trusted by <span className="font-medium text-white">500+</span> patients
                </p>
              </div>
            </div>
              ...
              <div className="relative w-full max-w-md aspect-square">
                <div className="absolute inset-0 rounded-2xl overflow-hidden bg-linear-to-br from-amber/10 to-sage/10 border border-white/10">
                  ...
                  <div className="absolute bottom-6 left-6 right-6">
                    <p className="text-white/80 text-sm font-medium">Independent Pharmacy</p>
