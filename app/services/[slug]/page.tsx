...
export default async function ServiceDetailPage({
  ...
  const getServiceColor = () => {
    ...
    return (
      colors[service.slug] || "bg-amber/10 text-amber-dark border-amber/20"
    );
  };

  const firstPost =
    service.facebookPosts && service.facebookPosts.length > 0
      ? service.facebookPosts[0]
      : null;
  ...
  const getHeroImage = () => {
    ...
    const images: Record<string, string> = {
      ...
      compounding: "/images/curemed/services/compounding.webp",
    };
    return (
      images[service.slug] ||
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 py-8 sm:py-12 lg:py-16">
        ...
        <div className="grid gap-8 lg:grid-cols-2 lg:gap-12 items-start">
          ...
          <div className="order-2 lg:order-1 space-y-5">
            <div className="flex items-center gap-3">
              <div
                className={`rounded-2xl ${getServiceColor()} p-2.5 border shadow-sm`}
              >
                {getServiceIcon()}
              </div>
              <span className="font-mono text-xs font-medium text-amber-dark bg-amber/10 px-3 py-1 rounded-full border border-amber/20">
                RX № {service.rx}
              </span>
            </div>
            ...
            <p className="text-base sm:text-lg leading-relaxed text-ink/70 max-w-2xl">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <span className="inline-flex items-center gap-1.5 rounded-full bg-amber/10 px-3 py-1.5 text-xs font-medium text-amber-dark border border-amber/20">
                <Award className="h-3.5 w-3.5" />
                Community Pharmacy
              </span>
              <span className="inline-flex items-center gap-1.5 rounded-full bg-blue-50 px-3 py-1.5 text-xs font-medium text-blue-700 border border-blue-200">
            ...
            <div className="flex flex-wrap gap-3 pt-1">
              <Link
                href="/contact"
                className="inline-flex items-center gap-2 rounded-full bg-amber px-6 py-2.5 font-mono text-xs font-medium uppercase tracking-wider text-paper transition-all hover:bg-amber-dark hover:scale-105 hover:shadow-lg"
              >
              ...
              {brand.social.facebook && (
                <a
                  ...
                  className="inline-flex items-center gap-2 rounded-full border border-ink/10 px-5 py-2.5 font-mono text-xs font-medium uppercase tracking-wider text-ink transition-all hover:border-[#1877F2]/30 hover:bg-[#1877F2]/5"
                >
                  <svg
        ...
        <div className="mt-12 grid gap-8 lg:grid-cols-[1.4fr_0.8fr]">
          ...
          <div className="space-y-6">
            ...
            {/* Highlights - تصميم جديد */}
            <div className="rounded-2xl border border-ink/10 bg-paper/60 p-6 sm:p-8">
              <h3 className="font-display text-lg font-semibold text-ink flex items-center gap-2 mb-5">
                <Sparkles className="h-5 w-5 text-amber-dark" />
                Key Benefits
              </h3>
              <div className="grid gap-3">
                {service.highlights.map((highlight, index) => (
                  <div
                    key={highlight}
                    className="flex items-start gap-3 p-3.5 rounded-xl bg-paper/50 border border-ink/5 hover:border-amber/10 hover:bg-paper/70 transition-all group"
                  >
                    <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-emerald-50 text-emerald-600 text-xs font-medium group-hover:scale-110 transition-transform">
                      {index + 1}
                    </span>
                    <span className="text-sm text-ink/70 group-hover:text-ink/90 transition-colors">
                      {highlight}
                    </span>
                  </div>
                ))}
          ...
          <aside className="space-y-4">
            ...
            <div className="rounded-2xl border border-amber/10 bg-linear-to-br from-amber/5 via-paper to-paper p-6 hover:shadow-md transition-shadow">
              ...
              <p className="text-sm leading-relaxed text-ink/70">
                Reach out to {brand.name} at{" "}
                <span className="font-medium text-ink">{brand.phone}</span> for
                questions about this service or to schedule a visit.
              </p>
              <Link
                href="/contact"
                className="mt-4 w-full inline-flex items-center justify-center gap-2 rounded-full bg-amber px-5 py-2.5 font-mono text-xs font-medium uppercase tracking-wider text-paper transition-all hover:bg-amber-dark hover:scale-[1.02] active:scale-95"
              >
                Contact Us
            ...
            <div className="rounded-2xl border border-ink/10 bg-paper/60 p-6">
              ...
              <div className="space-y-2 text-sm text-ink/60">
                ...
                <div className="flex justify-between items-center py-1 border-b border-ink/5">
                  <span>Walk-ins</span>
                  <span className="font-medium text-ink/80">Welcome</span>
                </div>
                <div className="flex justify-between items-center py-1">
                  <span>Service ID</span>
                  <span className="font-mono text-xs text-ink/50 bg-ink/5 px-2 py-0.5 rounded">
                    RX-{service.rx}
                  </span>
                </div>
              </div>
            ...
            <Link
              href="/services"
              className="block rounded-2xl border border-ink/5 bg-paper/40 p-4 text-center hover:bg-paper/80 transition-all hover:shadow-sm group"
            >
              <span className="text-sm text-ink/50 flex items-center justify-center gap-2 group-hover:gap-3 transition-all">
          ...
          <div className="mt-12 rounded-2xl border border-ink/10 bg-paper/60 p-6 sm:p-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {service.facebookPosts.map((post, index) => (
                <div
                  key={index}
                  className="group rounded-xl border border-ink/5 bg-paper/40 hover:border-amber/10 hover:shadow-md transition-all overflow-hidden"
                >
                  <a
                    href={
                      post.postId
                        ? `https://www.facebook.com/curemed/posts/${post.postId}`
                        : "#"
                    }
                    target="_blank"
                    rel="noopener noreferrer"
                  ...
                  >
                    {post.type === "video" && post.videoThumbnail ? (
                      ...
                      <div className="w-full h-full bg-sage/10 flex items-center justify-center">
                        <span className="text-sm text-ink/30">No image</span>
                      </div>
                    )}
                  ...
                  <div className="p-4">
                    ...
                    <p className="text-sm font-medium text-ink group-hover:text-amber-dark transition-colors line-clamp-2">
                      {post.title}
                    </p>
                    ...
                    {post.postId && (
                      <a
                        href={`https://www.facebook.com/curemed/posts/${post.postId}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="mt-2.5 inline-flex items-center gap-1 text-xs font-medium text-[#1877F2] hover:underline"
                      >
                        {post.type === "video" ? "Watch Video" : "View Post"}
                        <ArrowRight className="h-3 w-3" />
                      </a>
                    )}
                  </div>
            ...
            <div className="mt-6 text-center">
              <a
                href={`https://www.facebook.com/curemed`}
                target="_blank"
                rel="noopener noreferrer"
